...
We'll show that this fibration defines a completely integrable system in the rational case where $C = \bb{CP}^1$ with a framing at $\infty$.  We should first observe that the generic fibers are half-dimensional tori.  Computing the fibers of the Hitchin fibration works similarly to the non-singular case: a point in the base is, in particular, a map $C \bs D \to T/W$.  Suppose this map lands in the regular locus $T^{\mr{reg}}/W$, then the fiber over this point is, in particular, a map $C \bs D \to BT/W$.  \chris{... conditions at $D$ in the fiber, it'll be a $W$-equivariant $T$-bundle on the cameral cover but with some prescribed singularities at the preimage of $D$.  Also there's only one $T$-bundle on an affine curve, we want $T$-bundles on the compact curve which is the $W$-fold cover of $C$ ramified at $D$.}

\begin{remark}
\chris{Compare to the base in \cite{HurtubiseMarkman}.  Variety versus stack?  Difference in processing the $W$-action?}
\end{remark}

\chris{Include comparison to the story in \cite{DonagiGaitsgory}.}
...
Indeed, in the large $R$ limit these structures coincide as required. \chris{It would be better to make this argument more careful.}
\end{proof}
