...
There is a very extensive literature on the Yangian and related quantum groups.  For more information we refer the reader to \chris{References?  For instance \cite{CostelloYangian}, \cite{EtingofKazhdanIII} and \cite{ChariPressley}.}
...
\begin{theorem}
There is, for each choice $(D,\omega^\vee)$ of residue data, a deformation quantization $\OO_\hbar(\mhiggs^\fr_{SL_n}(\bb{CP}^1,D,\omega^\vee))$ of the algebra of functions on the multiplicative Higgs moduli space with the structure of a $Y(\sl_n)$-module.
\end{theorem}

\begin{proof}
\chris{I think this is a consequence of the combination of \cite{GKLO} (see also \cite{GKL}, though this doesn't talk about the classical limits / monopoles) and \cite{Shapiro}: The former describes the modules and computes their classical limits, then the latter proves that those classical limits are equivalent to the Smith Normal Form symplectic leaves.}
\end{proof}

The $Y(\gg)$-modules in question, as constructed in \cite{GKLO}, make sense for a general reductive group.  Although we don't have access to the Nahm transform for general $G$, so we can't repeat the above argument directly, it's still natural to make the following conjecture.

\begin{conjecture}
For any reductive group $G$, and any choice of residue data, the Poisson map $\OO(G_1[[z^{-1}]]) \to \OO(\mhiggs^\fr_G(\bb{CP}^1,D,\omega^\vee))$ quantizes to a $Y(\gg)$-module structure on a deformation quantization $\OO_\hbar(\mhiggs^\fr_G(\bb{CP}^1,D,\omega^\vee))$ of the algebra of functions on the multiplicative Higgs moduli space.  Specifically, this $Y(\gg)$-module is isomorphic to the module \chris{notation} defined in \cite{GKLO}.
\end{conjecture}
...
\[(\sp(4;\CC) \oplus \sp(2;\CC)_R \oplus V) \oplus \Pi(S \otimes W),\]
where $V$ is the five-dimensional defining representation of $\so(5;\CC) \iso \sp(4;\CC)$, $W$ is the two-dimensional defining representation of $\sp(2;\CC)_R$, and where there's an additional bracket $\Gamma \colon \sym^2(S \otimes W) \to V$ defined in the following way.  We can project $\sym^2(S \otimes W)$ onto $\wedge^2(S) \otimes \wedge^2(W)$.  The first factor decomposes as an $\sp(4;\CC)$-module into irreducible summands $V \oplus \CC$.  The second factor is isomorphic as an $\sp(2;\CC)_R$-module to $\CC$.  There is, therefore, a $\sp(4;\CC) \oplus \sp(2;\CC)_R$-equivariant projection $\wedge^2(S) \otimes \wedge^2(W) \to V \otimes \CC$ as required.
\end{definition}

Now, let's describe the locus of elements $Q \in S \otimes W$ such that $\Gamma(Q,Q) = 0$.  \chris{then twisting homomorphisms compatible with the relevant supercharges -- the ones with 4d image.}
