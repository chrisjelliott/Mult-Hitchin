...
In order to say this a bit more precisely this we'll compare our moduli space with the space of abstract Higgs bundles introduced by Donagi and Gaitsgory \cite{DonagiGaitsgory}.  Our argument will follow the same ideas as the arguments of \cite[Section 6]{HurtubiseMarkman}.
...
where $\mr{Cam}_G(C)$ is the stack of cameral covers of $C$, as defined in \cite[Section 2.8]{DonagiGaitsgory}.  The map $\mc B(C,D,\omega^\vee)_{\mr{reg}} \to \mr{Cam}_G(C)$ is defined by sending a meromorphic function $f \colon C \to T/W$ to the $D$-ramified cameral cover $\wt C = C \times_{T/W} T$.  In particular there's a map from the multiplicative Hitchin fiber to the corresponding Donagi-Gaitsgory fiber: the moduli space of abstract $G$-Higgs bundles with fixed cameral cover.  This map is surjective: having fixed the cameral cover, and therefore the ramification data, every sub-bundle $\mf c \sub \gg_P$ of regular centralizers arises as the centralizer of some regular multiplicative Higgs field.

To conclude this discussion we'll calculate dimensions.  In particular we'll see that the map onto the Donagi-Gaitsgory fiber is \'etale.  The Donagi-Gaitsgory fiber is, according to their main theorem, equivalent to the moduli space of $W$-equivariant $T$-bundles on the cameral curve $\wt C$ up to a discrete correction involving the root datum of $G$.  In particular it has dimension given by the product of the rank of $G$ and the genus of the cameral curve, which the Riemann-Hurwitz formula tells us is equal to the sum \chris{I'm a bit stuck here in computing the ramification degrees.  I'm also confused about the example of $\GL_2$ with two fundamental singularities.  So the moduli space there has dimension two, so the fiber should have dimension one, which is not divisible by the dimension of the maximal torus of $\GL_2$ as we'd expect.  Maybe this has to do with the fixed framing but I don't see it: fixing a framing should if anything lower the dimension of the fiber by two, and $1+2=3$ is still not divisible by two.}

On the other hand we can compute the dimension of a regular multiplicative Hitchin fiber by computing the dimension of the base and the dimension of the total space.  The dimension of the base is given by computing the number of linearly independent sections of the $T$-bundle $X(D,\omega^\vee)$ on $\bb{CP}^1$ vanishing at $\infty$.  This is given by 
...
Now, let's consider the large $R$ limit.  We'd like to note that the following difference of symplectic pairings converges to zero uniformly as $R\to \infty$.
...
\end{align*}
Since the moduli space of monopoles is compact \chris{wait, it isn't is it?  So maybe we need a different kind of convergence here} this converges to zero uniformly as $R \to \infty$.  The same calculation proves uniform convergence for the other symplectic structures in the hyperk\"ahler family, and therefore uniform convergence for the two complex structures.
\end{proof}
...
Let us proceed by discussing the connection between the symplectic structure on the moduli space of multiplicative Higgs bundles in the rational case and the rational Poisson Lie group.  This connection should be compared to the connection between the elliptic moduli space and the elliptic quantum group in the work of Hurtubise and Markman.  
...
\begin{proof}
We just need to compute the pullback of the Poisson bracket on $\OO(G_1[[z^{-1}]])$ under $r_\infty$. \chris{...}
\end{proof}

\chris{remark on the role played by the rational $r$-matrix?}
...
This comparison has interesting consequences upon quantization.  The Poisson algebra $\OO(G_1[[z^{-1}]])$ has a well-studied quantization to the \emph{Yangian} $Y(\gg)$.

\begin{definition}
\chris{define the Yangian, take care with completions.  References?}
\end{definition}

Likewise, we can study deformation quantization for the algebra $\OO(\mhiggs^\fr_G(\bb{CP}^1,D,\omega^\vee))$ of functions on our symplectic moduli space.  This moduli space is, in particular, a smooth finite-dimensional Poisson manifold, so its algebra of functions can be quantized using the theory
\chris{deformation quantization of functions on mHiggs, and of the Poisson map from $\OO(G_1[[z^{-1}]])$.  There's a possibility of an anomaly.  Probably the following should actually be a conjecture?}

\begin{corollary}
The quantum algebra of functions $\OO_\hbar(\mhiggs^\fr_G(\bb{CP}^1,D,\omega^\vee))$ has the structure of a $Y(\gg)$-module for each choice $(D,\omega^\vee)$ of residue data.
\end{corollary}

\chris{Include some discussion on how we can think of $\OO_\hbar(\mhiggs^{\text{fr,sing}}(\bb{CP}^1))$ as an intermediate Poisson Lie group between the Yangian and something dual to it.  References and a remark on the comparison with the conjectures made in \cite[Chapter 8.1]{NekrasovPestun}.  Include some words about $q$-Opers and $q$-W algebras, citing Sevostyanov et al and Aganagic-Frenkel-Okounkov \cite{AFO}.  Remark on the comparison with our modules and those of \cite{GKLO} -- would need some kind of Nahm transform story to relate our periodic monopoles with the Zastava spaces that appear in that work.}
