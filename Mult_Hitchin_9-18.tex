...
\begin{lemma}
There is a natural symplectic structure on the moduli space $\mhiggs_G^{\fr}(\bb{CP}^1,D,\omega^\vee)$.  In terms of our \v Cech description it is described by the formula 
...
\begin{remark}
How should we think about this symplectic structure?  There's an intuitive description just as in Hurtubise and Markman's elliptic moduli space.  The symplectic structure is induced from the natural equivalence between the tangent and cotangent spaces of the moduli space of multiplicative Higgs bundles as described in Section \ref{def_section}.  That is, there's a map of complexes of sheaves
\[\xymatrix{
...
\begin{proof}
\chris{todo.  It should be written so as to be manifestly anti-symmetric, we need to verify non-degeneracy and closedness/Jacobi.  This was somewhat tricky in HM.  Does the derived perspective help us?  Maybe we just shouldn't worry about closedness because it'll follow from the equivalence?  That relies on some precision there.}
\end{proof}

\begin{remark}
\chris{What about the Poisson structure on the total space?  These should be leaves.  Maybe just remark on this since we haven't calculated the tangent complex of the big space.  What about $\eps$-connections?}
\end{remark}
