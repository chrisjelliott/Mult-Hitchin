...
\chris{note: include citation / discussion of new paper by Costello and Yagi \cite{CostelloYagi}.}
...
\begin{example}
In the case where the group $G=\GL_n$ it's possible to describe the multiplicative Hitchin system on $\bb{CP}^1$ in terms of an \emph{ordinary} Hitchin system on $\CC^\times$, but for a different group $\GL_k$ and with different singularity data.  This relationship is given by the \emph{Nahm transform}.  From a gauge-theoretic point of view, the idea is that we can either interpret \chris{...}.  The Nahm transform in this setting was developed by Cherkis and Kapustin \cite{CherkisKapustin2}, and worked out in detail in the case $n=2$ (they also give a brane description of the transform: see \cite[Section 2]{CherkisKapustin2}).
