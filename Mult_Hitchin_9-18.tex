...
\begin{theorem}
Let $\left(\mhiggs^{\text{ps,fr}}_G(\bb{CP}^1,D,\omega^\vee)\right)_{\zeta,R}$ be the family of complex manifolds obtained by pulling back the holomorphic symplectic structure $\omega_{\zeta,R}$ on $\mon_G^{\mr{ps},\fr}(\bb{CP}^1 \times S^1_R,D,\omega^\vee)$ at $\zeta$ in the twistor sphere.  In the limit where $R \to \infty$ with $2 \pi \zeta R = \eps$ fixed this complex structure coincides with the complex structure on $\epsconn^{\text{ps,fr}}_G(\bb{CP}^1,D,\omega^\vee)$, in the sense that the complex structures converge pointwise over the tangent bundle to the moduli space of monopoles. \chris{can we / would we like to say anything stronger than pointwise convergence?  The symplectic forms aren't finite for any $L^p$ norm, I think, though we can say e.g. that we have uniform convergence after multiplying by some suitable, e.g. Schwartz function on the tangent space?}
\end{theorem}
...
Now, let's consider the large $R$ limit.  We'd like to note that the following difference of symplectic pairings converges to zero pointwise as $R\to \infty$.
...
This comparison has interesting consequences upon quantization.  The Poisson algebra $\OO(G_1[[z^{-1}]])$ has a well-studied quantization to the \emph{Yangian} $Y(\gg)$.  This is the unique Hopf algebra quantizing the algebra $U(\gg[z])$ with first order correction determined by the Lie bialgebra structure on $\gg[z]$.  Recall that $(\gg(\!(z)\!), \gg[z], \gg_1[[z^{-1}]])$ is a Manin triple, so the residue pairing on $\gg(!(z)\!)$ induces an isomorphism between $\gg_1[[z^{-1}]]$ and the dual to $\gg[z]$, and therefore a Lie cobracket on $\gg[z]$.  The uniqueness of this quantization is a theorem of Drinfeld \cite{DrinfeldQuantum}.

\begin{definition}
The \emph{Yangian} of the Lie algebra $\gg$ is the unique $\CC[[\hbar]]$-linear Hopf algebra $Y(\gg)$ where when we set $\hbar=0$ we recover $Y(\gg) \otimes_{\CC[[\hbar]]} \CC \iso U(\gg[z])$, and where the first order  \chris{... also References?  For instance \cite{CostelloYangian}, \cite{EtingofKazhdanIII} and \cite{ChariPressley}.}
\end{definition}

Likewise, we can study deformation quantization for the algebra $\OO(\mhiggs^\fr_G(\bb{CP}^1,D,\omega^\vee))$ of functions on our symplectic moduli space.  This moduli space is, in particular, a smooth finite-dimensional Poisson manifold, so its algebra of functions can be quantized, for instance using Kontsevich's results on formality.

\begin{definition}
The quantum algebra of functions $\OO_\hbar(\mhiggs^\fr_G(\bb{CP}^1,D,\omega^\vee))$ is \chris{deformation quantization: note the choices that need to be made.  Drinfeld associator / choice of formality equivalence?}
\end{definition}

\chris{There's a possibility of an anomaly for the Poisson morphism.  Probably the following should actually be a conjecture?  Can then discuss how Shapiro and GKLO verify the conjecture in some special cases (for GKLO: the case where all the singularities are at $0 \in \bb{CP}^1$).}
