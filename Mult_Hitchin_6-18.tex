...
\vasily{We should relate our work to the work by Donagi-Gaitsgory \url{https://arxiv.org/abs/math/0005132v3} on asbract Higgs bundles where they
  first define an `abstract Higgs bundle' over $X$ and then consider it valued in any abelian group scheme $K$ over $X$. A Higgs bundle valued in $K = T^{*} X$ produces the usual Hitchin system. The present work relates to Donagi-Gaitsgory construction by taking $X = \mathbb{P}^1$ and $K = X \times G_{m}$ where $G_{m}$ is multiplicative group, see definitions and explanations in sections 0.3.5 and 0.3.6 in that paper }
...
Write $M = C\times_q S^1_R$ for the $C$-bundle over $q$ with monodromy given by the automorphism $q$.  Fix the canonical Riemannian metric on $M$ associated to a choice of Riemannian \chris{K\"ahler?} metric on $C$ and the canonical metric on $S^1_R = \RR/2\pi R \ZZ$ \chris{todo, write more coherently.}
...
\begin{definition}
  ...
  \vasily{... and the restriction of a connection $A$ to a two-sphere $S^2$ enclosing the singularity defines a $U(1)$ bundle on this $S^2$ of degree $1$ so that
    \begin{equation}
...
The moduli theory of monopoles on 3-manifolds of this form has been studied in the mathematics literature by Foscolo \cite{FoscoloDef}, following earlier work  \vasily{need to add refs to Kapustin Cherkis and others before Foscolo}.  In the cases of interest to us it can be obtained as a hyperk\"ahler quotient.  Let's focus initially on the rational case, so monopoles on $M = \bb{CP}^1 \times_\eps S^1_R$ with Dirac singularities at $D \times \{t_0\}$ and a framing at $\infty$.  Consider the infinite-dimensional vector space $\mc V$ consisting of pairs $(A,\Phi)$ where $A$ is a connection on a fixed principal $G_\RR$-bundle $\bo P$ on $M$, $\Phi$ is a section of $\gg_{\bo P}$, and $(A,\Phi)$ have a Dirac singularity with charge $\omega^\vee_{z_i}$ at each $(z_i,t_0)$ in $D \times \{t_0\}$.  Let $\mc G$ be the group of gauge transformations of the bundle $\bo P$.
...
with differential $\d_A$ on each of the two factors along with the differential $\mc A$ \vasily{sorry, what is differential $\mc A$?} mixing the two factors \chris{careful here, need to justify how we get this}.  These two summands each split up into the sum of a Dolbeault complex on $C$ with its dual.  That is there's a natural subcomplex of the form
...
\vasily{Sorry, I am afraid I did not understand the notations of the remark and
  ...
  point corresponding to the split of $T_{M} \simeq \RR^3_{quat} \simeq \CC \oplus \RR = T_{C} \oplus T_{S^1}$ }
\end{remark}
...
\end{proof}
\vasily{I am not sure if I agree with the statement of the lemma (or may be I misunderstand),  and the proof, namely the wording "there is a unique such monopole by analytic continuation". 
I am confused by comparison with the fact that the moduli space of monopoles on a contractable space $\RR^3$ with a prescribed singularity $\omega^{\vee}$ at a single point $(z,t)$ is a non-trivial space of dimension $ 2 (\rho, \omega^{\vee})$ studied by BFN and Kamnitzer et all in the papers about slices in affine Grassmanian. 
If we replace $\RR^3$ by a ball $U$ with a reasonable boundary conditions like Dirichlet or Neumann, we would still get a non-trivial moduli space
of monopoles of dimension $2 (\rho, \omega^{\vee})$ 
}
...
\vasily{Does the proof above applies to the case of generic $q$ or for the trivial $q$?} 


\begin{prop} \label{local_derivative_description_prop}
The derivative $\d H \colon \bb H^\bullet(U_i ; \mc F'_{(\bo P,\mc A)}) \to \bb H^\bullet(\bb D_i; \mc F_{(P,g)})$ is given on an open patch $U_i$ by the formula
...
\[\lambda = f_1^*(\mu_1) + f_2^*(\mu_2) + \eta
\]
where $\eta$ is a potential for the pullback $(f_1 \circ g)^*\nu$.  The Lagrangian structure $\mu_1$ on the diagonal is 0, and the Lagrangian structure $\mu_2$ vanishes upon restriction to the open set $U_i$ since the map $\bun_G^{\fr}(\bb{CP}^1 \bs D) \to \bun_G^{\fr}(\bb{CP}^1 \bs D)^2 \times_{BLG^{2k}} BLG^{k}$ becomes an isomorphism after restricting to a formal neighbourhood of $z_i$ \chris{justify the difference between a formal neighbourhood and a contractible analytic neighbourhood here}.  We must therefore find a local potential for $(f_1 \circ g)^*\nu$ where $\nu$ is the structure from Lemma \ref{restriction_Lagn_lemma}.  On the cotangent complex, the pullback $g^*$ acts by the dual to the $q$-twisted diagonal map $\gg[[z]] \to \gg(\!(z)\!)^2$.  The pullback $f_1^*$ acts by the dual to the projection map $\gg(\!z)\!)^2 \to \ad(g)_{\bb D^\times}$.  The composite is therefore given by $\Ad_g^*$. \chris{sign error here.  There's also another degree in the cotangent complex to calculate: map should be also $\Ad_g$ but without the dual?}

On the other hand, applying the internal differential induced from $\Ad_g^*$ on the complex of 2-forms to the local pairing $\omega$ yields the degree 1 pairing
\begin{align*}
\langle (\alpha_i,\beta_i), (\alpha_i',\beta_i') \rangle &= \langle \rho_g^*(\Ad_g(\beta_i)), \rho_g^*(\Ad_g^*)^{-1}(\beta'_i) \rangle \\
&= \langle \rho_g^*(\beta_i), \rho_g^*(\beta'_i) \rangle.
\end{align*}
We can then take the pairing defining $\eta$ from Lemma \ref{restriction_Lagn_lemma} and pull it back along $f_1 \circ g$ to obtain the local description for the pairing
\begin{align*}
\langle (\alpha_i,\beta_i), (\alpha_i',\beta_i') \rangle &= \langle \rho_g^* ... , \rho_g^* ... \rangle 
\end{align*}
\chris{...}
\end{proof}
...
\begin{align*}
\omega(\delta g, \delta g') &= \sum_{i=1}^d  - \langle \rho_g^*A_g b^L_i, \rho_g^*b^{'R}_i \rangle - \langle \rho_g^*b^L_i, \rho_g^*A_{g^{-1}}b^{'R}_i \rangle- (b \leftrightarrow b') \\ 
...
\end{align*}
We can write this in terms of a pairing on the bundle $\ad(g)$.  Represent the class $b_i(2\pi)H(\mc A) - H(\mc A)b_i(0)$ by the pair $(b_i(2\pi)H(\mc A), -H(\mc A)b_i(0))$.  Then applying the description of $\omega_{\mr{mHiggs}}$ provided by Proposition \ref{qconn_symp_description} we have
\begin{align*}
...
\vasily{ 1. let us unify the notations for the Higgs field in the equations above and the previous sections: somewhere holonomy or
  ...
  the lines the explanations of what we are doing  }
...
\end{align*}
On the second line we used the fact that $\d_{\mr{mon}}b_i = \d_{\mr{mon}} b_0$ on $U_i \cap U_0$ and that $\d_{\mr{mon}} b_i(t) = 0$ when $t = 0$ or $2\pi$.  Pick out the summand corresponding to $U_i$.  Choose our potentials so that on the boundary $\dd \bb D_i$ we have $b_0(0) = 0$ and $b_i(2\pi) = 0$ \chris{possible why?},
\vasily{Instead, I would suggest that here we choose the potentials such
  that $b_0(0) = 0$ and $b_0'(0) = 0$ everywhere on $U_0$,
...
$b_i^{R} b_{i}^{R}{}'$ vanishes and we are left with
 ...
 which means that on $U_0 \cap U_i$ we can identify $b_0(2\pi) = H(\mc A) b_i(0) H(\mc A)^{-1}$.  Then
\[ \omega_{\mr{Mon}}(\{\alpha_i\},\{\alpha'_i\}) = \oint_{\dd \bb D_i} \kappa( b_i(2\pi),  H(\mc A)b_i'(0) H(\mc A)^{-1}) - (b \leftrightarrow b')\]
