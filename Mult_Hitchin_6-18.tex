...
\chris{There are a lot of notes in red: that just indicates that there are a lot of arguments that are only partially written.  They're notes for myself.  Testing git here.}
...
\end{align*}
using cyclic invariance in the last step. \chris{Still not the formula that we want exactly.  Want something more like 
\[\sum_{i=1}^d \langle  b_i(2\pi) \phi, \phi b'_i(0) \rangle + \langle b'_i(2\pi) \phi, \phi b_i(0) \rangle\]
to compare with what we have below.  Even so there are left-left and right-right cross-terms to kill, plus sign issues.}
\end{proof}
...
\end{align*}
\chris{should be an easy calculation to finish once I fix the proposition above.}
\end{proof}
