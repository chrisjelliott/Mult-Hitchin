...
\vasily{We've defined $q$-connection as a holonomy around the complete circle 
$S^1$ (from $0$ to $2 \pi$ in the $t$ coordinates). But in the paragraph above
we need to intertwine by a morphism from a bundle $P$ defined at a curve $C$ at 
 $t=0$ to a bundle defined at a curve $C$ at arbitrary $t$?  }
...
\end{proof}
\vasily{Sorry, I did not get the last sentence.  The statement about a second order   zero at $\infty$ applies for any $t$? What is the boundary conditions at $t=0$ for the heat equations, or it does not matter?Also, to be sure, the $t$ on this page is not the $t$ that parametrizes the $S^1$ circle on the previous page, correct? }
